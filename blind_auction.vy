...
# Note: because Vyper does not allow for dynamic arrays,
# the number of bids have been limited to 128 per address
...
# Create a blinded auction with `_biddingTime` seconds
# bidding time and `_revealTime` seconds reveal time
# on behalf of the beneficiary address `_beneficiary`
@external
def __init__(_beneificary: address, _biddingTime: uint256, _revealTime: uint256):
...
def bid(_blindedBid: bytes32):
  # Check if the bidding period is still open
  assert block.timestamp < self.biddingEnd

  # Check that payer hasn't already placed maximum number
  # of bids
...
# Returns a boolean value, `True` if bid placed successfully
# `False` otherwise.
@internal
...
def reveal(_numBids: int128, _values: uint256[128], _fakes: bool[128], _secrets: bytes32[128]):
  ...
  # Check that reeal end has not passed
  ...
  for i in range(MAX_BIDS):
    # Note that loop may break sooner than 128 iterations
    # if i >= _numBids
    if (i >= _numBids):
      break
      ...
      # Make it impossible for sender to re-claim the
      # same deposit
...
def withdraw():
  ...
  if (pendingAmount > 0):
    # If so, set pending returns to zero to prevent recipient
    # from calling this function again as part of the
    # receiving call before `transfer` returns
